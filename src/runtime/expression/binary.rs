...
pub async fn evaluate_binary<'a>(
    ...
    match binary_expression.operator {
        ...
        BinaryOperator::Subtraction => match (left_value, right_value) {
            (Value::Integer(l), Value::Integer(r)) => l
                .checked_sub(r)
                .map(Value::Integer)
        ...
        BinaryOperator::Multiplication => match (left_value, right_value) {
            (Value::Integer(l), Value::Integer(r)) => l
                .checked_mul(r)
                .map(Value::Integer)
        ...
        BinaryOperator::Division => match (left_value, right_value) {
        ...
        BinaryOperator::Remainder => match (left_value, right_value) {
            (Value::Integer(l), Value::Integer(r)) => {
                if r == 0 {
        ...
        BinaryOperator::Exponent => match (left_value, right_value) {
            ...
            (l, r) => Err(format!(
                "Cannot use ** operator on {l:?} with exponent {r:?}"
            )),
        },
        ...
        | BinaryOperator::LessOrEqual => {
            ...
            match ordering {
                Some(std::cmp::Ordering::Less) => Ok(Value::Bool(matches!(
                    binary_expression.operator,
                    BinaryOperator::Less | BinaryOperator::LessOrEqual
                ))),
                Some(std::cmp::Ordering::Equal) => Ok(Value::Bool(matches!(
                    binary_expression.operator,
                    BinaryOperator::Equal
                        | BinaryOperator::GreaterOrEqual
                        | BinaryOperator::LessOrEqual
                ))),
                Some(std::cmp::Ordering::Greater) => Ok(Value::Bool(matches!(
                    binary_expression.operator,
